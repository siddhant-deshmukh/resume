...
//     if (event.target) {
//       const url = new URL(location);
//       url.searchParams.set("section", event.target.name);

//       history.pushState({ project: event.target.name }, '', url)
//       updateButton()
//     }
//   })
// })
...
//       // console.log(stack[-1], entry.target.id)
//       // if(stack[-1] === entry.target.id){
//       //   stack.pop()
//       // }
//       // console.log(entry.target.id , 'not intersecting')
//       sections.push(entry.target.id)
//     } 
//     // else {
//     //   stack.push(entry.target.id)
//     //   console.log("Interecting" , entry.target.id)
//     // }
//   })
//   console.log(sections)
//   navBtn.forEach((btn)=>{
//     if(sections[0] === btn.name){
//       btn.classList.add('choosen')
//       btn.classList.remove('not-choosen')
//     }else {
//       btn.classList.remove('choosen')
  ...
  if (projectType && projectType !== 'all') {
    for(let i=0; i< projectElements.length; i++){
      ...
      if(tags.indexOf(projectType) != -1){
        projectElements[i].classList.add('temp-visible')
        projectElements[i].classList.remove('temp-hide')
      } else {
        projectElements[i].classList.remove('temp-visible')
        projectElements[i].classList.add('temp-hide')
      }
    }
  } else {
    for(let i=0; i< projectElements.length; i++){
